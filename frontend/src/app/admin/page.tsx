// ==============================================================================
// Admin Control Panel Page Component (Diagnostic & Audit Logs Dashboard)
// ==============================================================================
  ...
  const { data: stats, isLoading: isStatsLoading, refetch: refetchStats } = useQuery<SystemStats>({
    ...
    queryFn: async () => {
      try {
        ...
        // Fallback mock stats for sandboxed run
        return {
          system: {
            uptime: 12450,
            cpuLoad: "1.2%",
    ...
    queryFn: async () => {
      try {
        ...
        return [
          ...
          { id: "log-5", action: "SELL_ORDER_FILLED", description: "Sold 30 shares of AAPL at $185.74", ipAddress: "192.168.1.1", timestamp: new Date(Date.now() - 8 * 3600 * 1000).toISOString(), user: { email: "demo@marketmind.ai", role: "USER" } }
        ];
      }
    }
  });
      ...
      <LayoutShell>
        <div className="max-w-7xl mx-auto space-y-6">
          ...
          <div className="flex items-center justify-between">
            <div>
              <span className="text-[10px] font-bold text-indigo-400 uppercase tracking-widest">CONTROL PORTAL</span>
            ...
            <button
              ...
              className="flex items-center gap-2 px-3 py-1.5 rounded-xl bg-white/5 border border-border text-xs text-white hover:bg-white/10 transition-all cursor-pointer"
            >
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
            ...
            <div className="glass-card p-6 rounded-2xl border border-border space-y-3">
              <div className="flex items-center justify-between">
                <span className="text-[10px] font-bold text-indigo-400 uppercase tracking-widest">PLATFORM METRICS</span>
                <Activity className="w-4 h-4 text-indigo-400" />
              </div>
              <div className="space-y-1 text-xs">
                <div className="flex justify-between"><span className="text-muted-foreground">Portfolios:</span><span className="text-white font-mono">{stats?.database.portfolios}</span></div>
                <div className="flex justify-between"><span className="text-muted-foreground">Orders Filled:</span><span className="text-white font-mono">{stats?.database.ordersFilled}</span></div>
                <div className="flex justify-between"><span className="text-muted-foreground">Redis Cache:</span><span className="text-green-400 font-semibold">ONLINE</span></div>
              </div>
            </div>
